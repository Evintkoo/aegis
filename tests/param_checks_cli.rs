...
fn explicit_param_finds_reflected_xss() {
    ...
    assert!(
        stdout.contains("Reflected XSS") || stdout.contains("XSS"),
...
fn crawl_discovers_and_fuzzes_a_linked_parameter() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("fuzzing GET") && stdout.contains("id") && stdout.contains("via link"),
        "expected discovery to fuzz the linked 'id' param, got:\n{stdout}"
    );
}

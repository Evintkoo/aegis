...
const COMMON: &[&str] = &["next", "url", "return", "returnTo", "redirect", "redirect_uri", "dest", "continue"];
...
fn location_host(loc: &str) -> String {
    if let Some(idx) = loc.find("://") {
        loc[idx + 3..].split(['/', '?', '#']).next().unwrap_or("").to_lowercase()
...
async fn check_param(client: &HttpClient, param: &str, method: &str) -> Option<(String, String)> {
    ...
    for pl in payloads {
        let req = if method == "GET" { HttpRequest::get().param(param, pl) } else { HttpRequest::post().form_field(param, pl) };
        let Ok(r) = client.request(req.no_redirects()).await else { continue };
        let loc = r.headers.iter().find(|(k, _)| k.eq_ignore_ascii_case("Location")).map(|(_, v)| v.clone()).unwrap_or_default();
        let host = location_host(&loc);
        if host.contains("evil.example.com") || loc.starts_with("//evil") || loc.starts_with("/\\evil") {
...
async fn run_impl(client: &HttpClient, opts: &Opts) -> Vec<Finding> {
    ...
    for param in params {
        if let Some((pl, loc)) = check_param(client, param, &method).await {
            out.push(
                Finding::new(NAME, Severity::Medium, "Open redirect", format!("param '{param}' redirects off-site")).with_evidence(format!("{pl} -> {loc}")),
            );
...
mod tests {
    ...
    async fn detects_protocol_relative_bypass() {
        ...
        assert!(findings.iter().any(|f| f.evidence.starts_with("//evil.example.com/pwn ->")));
    }

    #[tokio::test]
    async fn no_findings_when_redirects_stay_on_site() {
        let base = scripted_server(|_req, _| ScriptedResponse::with_status(302, "").header("Location", "/home")).await;
        let client = fast_client(base);
        let opts = Opts { param: Some("next".to_string()), ..Opts::default() };

...
async fn run_impl(client: &HttpClient, opts: &Opts) -> Vec<Finding> {
    ...
    let base = if opts.base_value.is_empty() { "x" } else { &opts.base_value };
    ...
    for (payload, marker) in PROBES {
        ...
        if r.body.contains(marker) && !baseline.body.contains(marker) && !r.body.contains(payload) {
            let mut f = Finding::new(NAME, Severity::Critical, "Server-Side Template Injection", format!("payload {payload:?} evaluated to {marker:?}"))
...
mod tests {
    ...
    async fn detects_jinja_style_evaluation() {
        ...
        let opts = Opts { param: Some("name".to_string()), ..Opts::default() };
    ...
    async fn no_finding_when_marker_appears_but_payload_is_also_reflected_verbatim() {
        ...
        assert!(findings.is_empty(), "marker text coinciding with a verbatim-reflected payload must not count as SSTI");
    }

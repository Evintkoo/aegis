...
async fn run_impl(client: &HttpClient, _opts: &Opts) -> Vec<Finding> {
    ...
    if let Ok(tr) = client.request(HttpRequest { method: Method::TRACE, ..HttpRequest::get() }.header("X-Xst-Probe", "trace-canary-9182")).await {
        if tr.body.contains("trace-canary-9182") && tr.status < 400 {
            out.push(
                Finding::new(NAME, Severity::Medium, "HTTP TRACE enabled (XST)", "server echoes the request — enables Cross-Site Tracing")
                    .with_evidence(tr.body.chars().take(80).collect::<String>().replace('\n', " ")),
            );
        }
    }

    // 2) WebDAV PUT write (to a throwaway path), then clean up
    if let Ok(pr) = client.request(HttpRequest { method: Method::PUT, url: Some(probe_path.clone()), ..HttpRequest::get() }.raw_body(b"pentest-write-probe".to_vec())).await {
        if matches!(pr.status, 200 | 201 | 204) {
            out.push(
                Finding::new(NAME, Severity::High, "HTTP PUT allows file write (WebDAV)", format!("PUT created {probe_path} (HTTP {}) — remote file upload", pr.status))
                    .with_evidence(format!("HTTP {}", pr.status)),
            );
            if client.request(HttpRequest { method: Method::DELETE, url: Some(probe_path.clone()), ..HttpRequest::get() }).await.is_err() {
                out.push(Finding::new(NAME, Severity::Info, "Could not auto-delete PUT probe file", format!("manually remove {probe_path}")));
            }
        }
    }
    ...
    let override_result = client
        .request(HttpRequest::post().header("X-HTTP-Method-Override", "PUT").header("X-HTTP-Method", "PUT").raw_body(b"probe".to_vec()))
        .await;
...
mod tests {
    ...
    async fn detects_trace_echoing_the_probe_header() {
        let base = scripted_server(|req, _| {
            if req.path == "/" && req.headers.iter().any(|(k, v)| k.eq_ignore_ascii_case("X-Xst-Probe") && v == "trace-canary-9182") {
        ...
        assert!(findings.iter().any(|f| f.title == "HTTP TRACE enabled (XST)"));
    }
    ...
    async fn detects_put_write_and_cleans_up() {
        ...
        assert!(findings.iter().any(|f| f.title == "HTTP PUT allows file write (WebDAV)"));
        assert!(!findings.iter().any(|f| f.title == "Could not auto-delete PUT probe file"));
    }

    #[tokio::test]
    async fn detects_method_override_bypass() {
        let base = scripted_server(|req, _| {
            let overridden = req.headers.iter().any(|(k, v)| k.eq_ignore_ascii_case("X-HTTP-Method-Override") && v == "PUT");
            if overridden {
                ScriptedResponse::ok("override honored")
            } else if req.headers.iter().any(|(k, _)| k.eq_ignore_ascii_case("X-Xst-Probe")) {
        ...
        assert!(findings.iter().any(|f| f.title == "Method-override header honored"));
    }

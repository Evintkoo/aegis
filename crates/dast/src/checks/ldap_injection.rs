...
const PAYLOADS: &[&str] = &["*", "*)(uid=*))(|(uid=*", "*)(|(objectclass=*", "admin)(&))", ")(cn=*", "*))%00", "*()|&'"];
    ...
    for p in PAYLOADS {
        ...
        if let Some(m) = ERROR_MARKERS.find(&r.body) {
            if ERROR_MARKERS.find(&baseline.body).is_none() {
                out.push(Finding::new(NAME, Severity::High, "LDAP injection (error-based)", format!("payload {p:?} triggered an LDAP error")).with_evidence(m.as_str()));
                return out;
            }
        }
        // `*` widening: response grows substantially / diverges but stays 2xx
        if *p == "*" && r.status < 400 && similarity(&r.body, &baseline.body) < 0.85 && r.body.len() > (baseline.body.len() as f64 * 1.2) as usize {
            out.push(
                Finding::new(NAME, Severity::High, "Possible LDAP injection (wildcard widened results)", "'*' returned a larger/different result set")
                    .with_evidence(format!("len {}->{}", baseline.body.len(), r.body.len())),
            );
            return out;
        }
    }
...
mod tests {
    ...
    async fn detects_error_based_injection() {
        ...
        let opts = Opts { param: Some("uid".to_string()), ..Opts::default() };
    ...
    async fn detects_wildcard_widened_results() {
        ...
        assert_eq!(findings.len(), 1);
        assert_eq!(findings[0].title, "Possible LDAP injection (wildcard widened results)");
    }

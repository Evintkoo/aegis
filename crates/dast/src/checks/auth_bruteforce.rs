...
static LOCKOUT_RE: LazyLock<regex::Regex> = LazyLock::new(|| {
    regex::Regex::new(r"(?i)(too many|rate limit|locked|lockout|try again later|captcha|temporarily (disabled|blocked)|slow down|429)").unwrap()
});
...
async fn login(client: &HttpClient, opts: &Opts, user: &str, pw: &str) -> Option<HttpResponse> {
    ...
    let req = if opts.auth_json {
        ...
        map.insert(
            opts.user_field.clone(),
            serde_json::Value::String(user.to_string()),
        );
        map.insert(
            opts.pass_field.clone(),
        ...
        HttpRequest::post()
            .url(url)
            .form_field(&opts.user_field, user)
            .form_field(&opts.pass_field, pw)
    };
...
async fn run_impl(client: &HttpClient, opts: &Opts) -> Vec<Finding> {
    ...
    let mut out = Vec::new();
    let bogus_user = format!(
        "zzq_nouser_9182_{}",
        valid_user.chars().take(4).collect::<String>()
    );
    let wrong_pw = "definitely_wrong_pw_9182!";
    let attempts = opts.auth_attempts.min(5);

    // --- 1) User enumeration -------------------------------------------------
    let Some(r_valid) = login(client, opts, &valid_user, wrong_pw).await else {
        return out;
    };
    let Some(r_bogus) = login(client, opts, &bogus_user, wrong_pw).await else {
        return out;
    };

    if let Some(m) = NOUSER_RE
        .find(&r_bogus.body)
        .filter(|_| !NOUSER_RE.is_match(&r_valid.body))
    {
        out.push(
            Finding::new(
                NAME,
    ...
    for i in 0..attempts {
        let Some(r) = login(client, opts, &valid_user, &format!("{wrong_pw}{i}")).await else {
            continue;
        };
...
mod tests {
    ...
    async fn detects_enumeration_via_distinct_error_message() {
        ...
        let client = fast_client(base);
        let opts = Opts {
            login_url: Some(format!("{}/login", client.base_url())),
        ...
        assert!(findings
            .iter()
            .any(|f| f.title == "Username enumeration (distinct error message)"));
    }
    ...
    async fn detects_missing_rate_limiting() {
        ...
        assert!(findings
            .iter()
            .any(|f| f.title == "No rate limiting / account lockout on login"
                && f.evidence.contains("3 attempts")));
    }

    #[tokio::test]
    async fn no_lockout_finding_when_the_app_throttles() {
        let base =
            scripted_server(|_req, _| ScriptedResponse::with_status(429, "too many requests"))
                .await;
        let client = fast_client(base);
        let opts = Opts {
            login_url: Some(format!("{}/login", client.base_url())),

...
async fn run_impl(client: &HttpClient, opts: &Opts) -> Vec<Finding> {
    ...
    if method == "GET" {
        ...
        let probes: [(HttpRequest, &str); 4] = [
            (HttpRequest::get().param(format!("{param}[$ne]"), base), "operator $ne injection"),
            (HttpRequest::get().param(format!("{param}[$gt]"), ""), "operator $gt injection"),
            (HttpRequest::get().param(format!("{param}[$regex]"), ".*"), "operator $regex injection"),
            (HttpRequest::get().param(param, format!("{base}' || '1'=='1")), "JS boolean injection"),
        ];
        for (req, why) in probes {
            let Ok(r) = client.request(req).await else { continue };
            if let Some(m) = ERROR_MARKERS.find(&r.body) {
                out.push(Finding::new(NAME, Severity::High, "NoSQL injection (error-based)", why).with_evidence(m.as_str()));
                return out;
            }
            let sim = similarity(&r.body, &baseline.body);
            if sim < 0.85 && r.status < 500 {
                out.push(
                    Finding::new(NAME, Severity::High, "Possible NoSQL injection (behavior change)", format!("{why} altered response"))
                        .with_evidence(format!("sim={sim:.2}")),
                );
                return out;
            }
        }
    } else {
        let Ok(baseline) = client.request(HttpRequest::post().json(serde_json::json!({ param: base }))).await else {
            return out;
        };
        let probes: [(serde_json::Value, &str); 3] = [
            (serde_json::json!({ param: { "$ne": serde_json::Value::Null } }), "{$ne:null} auth-bypass"),
            (serde_json::json!({ param: { "$gt": "" } }), "{$gt:''} operator"),
            (serde_json::json!({ param: { "$regex": ".*" } }), "{$regex:'.*'} operator"),
        ];
        for (body, why) in probes {
            let Ok(r) = client.request(HttpRequest::post().json(body)).await else { continue };
            if let Some(m) = ERROR_MARKERS.find(&r.body) {
                out.push(Finding::new(NAME, Severity::High, "NoSQL injection (error-based)", why).with_evidence(m.as_str()));
                return out;
            }
...
mod tests {
    ...
    async fn detects_operator_injection_error_based_over_get() {
        let base = scripted_server(|req, _| {
            if req.query.keys().any(|k| k.contains("$ne") || k.contains("$gt") || k.contains("$regex")) {
        ...
        let opts = Opts { param: Some("user".to_string()), ..Opts::default() };
    ...
    async fn detects_auth_bypass_over_post_json() {
        ...
        .await;
        let client = fast_client(base);
        let opts = Opts { param: Some("password".to_string()), method: "POST".to_string(), ..Opts::default() };

...
static STACK_MARKERS: LazyLock<Vec<(regex::Regex, Severity, &'static str)>> = LazyLock::new(|| {
    vec![
        ...
        (
            regex::Regex::new(r"at [\w.$]+\([\w.]+\.java:\d+\)").unwrap(),
            Severity::High,
            "Java stack trace",
        ),
        (
            regex::Regex::new(r"You're seeing this error because you have DEBUG = True").unwrap(),
            Severity::High,
            "Django DEBUG page",
        ),
        (
            regex::Regex::new(r"Whitespace-sensitive.*Rails|Action Controller: Exception caught")
                .unwrap(),
            Severity::High,
            "Rails error page",
        ),
        (
            regex::Regex::new(r"Fatal error:.*on line \d+").unwrap(),
            Severity::High,
            "PHP fatal error",
        ),
        (
            regex::Regex::new(r"Warning: .* in .* on line \d+").unwrap(),
            Severity::Medium,
            "PHP warning w/ path",
        ),
        (
            regex::Regex::new(r"System\.\w+Exception:").unwrap(),
            Severity::High,
            ".NET exception",
        ),
        (
            regex::Regex::new(r"ORA-\d{5}|SQLSTATE\[").unwrap(),
            Severity::Medium,
            "DB error string",
        ),
        (
            regex::Regex::new(r"node_modules|/var/www/|/home/\w+/|C:\\\\").unwrap(),
            Severity::Low,
            "internal filesystem path leak",
        ),
    ]
});

static SECRET_COMMENT_RE: LazyLock<regex::Regex> = LazyLock::new(|| {
    regex::Regex::new(
        r"(?i)<!--[^>]*(password|passwd|secret|api[_-]?key|todo|fixme|hack|xxx)[^>]*-->",
    )
    .unwrap()
});
static KEY_RE: LazyLock<regex::Regex> = LazyLock::new(|| {
    regex::Regex::new(r"(AKIA[0-9A-Z]{16}|-----BEGIN (RSA|EC|OPENSSH) PRIVATE KEY-----|ghp_[A-Za-z0-9]{36}|xox[baprs]-[A-Za-z0-9-]+)").unwrap()
});
            ...
            out.push(
                Finding::new(
                    ...
                    *sev,
                    ...
                    format!("verbose error/debug output in {where_}"),
                )
    ...
    for m in SECRET_COMMENT_RE.find_iter(body) {
        out.push(
            Finding::new(
                ...
                format!("comment may leak info in {where_}"),
            )
            .with_evidence(m.as_str().chars().take(120).collect::<String>()),
        );
    }
    for m in KEY_RE.find_iter(body) {
        out.push(
            Finding::new(
                NAME,
                Severity::Critical,
                "Hard-coded credential/key in response",
                format!("secret pattern found in {where_}"),
            )
            .with_evidence(format!(
                "{}…",
                m.as_str().chars().take(20).collect::<String>()
            )),
        );
    }
}
...
async fn run_impl(client: &HttpClient, opts: &Opts) -> Vec<Finding> {
    ...
    if let Ok(err) = client
        .request(HttpRequest::get().url(format!("{root}/zzq_nonexistent_%27%22")))
        .await
    {
    ...
    if let Some(param) = &opts.param {
        ...
        let req = if method == "GET" {
            HttpRequest::get().param(param, "'\"><")
        } else {
            HttpRequest::post().form_field(param, "'\"><")
        };
...
mod tests {
    ...
    async fn detects_python_stack_trace_on_the_base_page() {
        let base = scripted_server(|_req, _| {
            ScriptedResponse::with_status(500, "Traceback (most recent call last):\n  File x")
        })
        .await;
        ...
        assert!(findings
            .iter()
            .any(|f| f.title == "Python stack trace exposed"));
    }
    ...
    async fn detects_a_leaked_aws_key_and_dedupes_repeats() {
        ...
        let hits: Vec<_> = findings
            .iter()
            .filter(|f| f.title == "Hard-coded credential/key in response")
            .collect();
    ...
    async fn scans_the_param_error_response_when_a_param_is_supplied() {
        ...
        assert!(findings
            .iter()
            .any(|f| f.title == "DB error string exposed"));
    }

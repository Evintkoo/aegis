...
const ENDPOINTS: &[&str] = &["/graphql", "/api/graphql", "/v1/graphql", "/query", "/graphql/console", "/graphiql", "/playground", "/api/graphql/v1"];
...
async fn run_impl(client: &HttpClient, _opts: &Opts) -> Vec<Finding> {
    ...
    for ep in ENDPOINTS {
        ...
        let Ok(r) = client.request(HttpRequest::post().url(&url).json(serde_json::json!({"query": "{__schema{types{name}}}"}))).await else {
            continue;
        };
        if r.status == 200 && r.body.contains("__schema") {
            if let Ok(data) = serde_json::from_str::<serde_json::Value>(&r.body) {
                if let Some(types) = data.get("data").and_then(|d| d.get("__schema")).and_then(|s| s.get("types")).and_then(|t| t.as_array()) {
                    if !types.is_empty() {
                        out.push(
                            Finding::new(NAME, Severity::High, "GraphQL introspection enabled", format!("{ep} exposes full schema ({} types)", types.len()))
        ...
        let Ok(g) = client.request(HttpRequest::get().url(&url)).await else { continue };
        let lower = g.body.to_lowercase();
        if g.status == 200 && (lower.contains("graphiql") || lower.contains("playground")) {
            out.push(Finding::new(NAME, Severity::Medium, "GraphQL IDE exposed", format!("{ep} serves an interactive GraphQL IDE")).with_evidence(ep.to_string()));
            return out;
        }
    }
    out
    ...
    async fn detects_introspection_enabled() {
        let base = scripted_server(|req, _| {
            if req.path == "/graphql" && req.body.contains("__schema") {
                ScriptedResponse::ok(r#"{"data":{"__schema":{"types":[{"name":"Query"},{"name":"User"}]}}}"#)
            } else {
        ...
        assert!(findings.iter().any(|f| f.title == "GraphQL introspection enabled" && f.detail.contains("2 types")));
    }
